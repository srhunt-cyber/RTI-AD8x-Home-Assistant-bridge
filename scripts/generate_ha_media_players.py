...
def _source_maps(amp: dict[str, Any], use_names: bool) -> tuple[dict[str, str], dict[str, str]]:
    """Return RTI-number-to-label and label-to-RTI-number source maps."""
    number_to_label = {
        str(number): amp["sources"][number] if use_names else f"INPUT {number}"
        for number in sorted(amp["sources"])
    }
    return number_to_label, {label: number for number, label in number_to_label.items()}
...
def build_package(config: dict[str, Any], only: list[str] | None = None) -> dict[str, Any]:
    ...
    for amp, zone in _selected_zones(config, only):
        ...
        number_to_label, label_to_number = _source_maps(amp, use_names)
        source_options = list(number_to_label.values())
        number_to_label_json = json.dumps(number_to_label)
        label_to_number_json = json.dumps(label_to_number)
        template_sensors.append({
            "name": f"RTI MP {amp_id} {zone_name} Source",
            "unique_id": f"{key}_source",
            "state": (
                f"{{% set raw = states('{source_raw_entity}') %}} "
                f"{{% set source_map = {number_to_label_json} %}} "
                "{{ source_map.get(raw, raw) }}"
            ),
        ...
        players.append({
            ...
            "commands": {
                ...
                "select_source": {
                    "action": "mqtt.publish",
                    "data": {
                        "topic": f"{command_base}/source",
                        "payload": (
                            f"{{% set source_map = {label_to_number_json} %}} "
                            "{{ source_map.get(source, source | upper "
                            "| replace('INPUT ', '') | trim) }}"
                        ),
                    },
                },
...
def build_alexa_cloud_config(
    config: dict[str, Any], only: list[str] | None = None
) -> dict[str, Any]:
    """Build an include-file snippet that marks generated players as music systems."""
    suffix = config["home_assistant"]["media_players"]["name_suffix"]
    entity_config = {}
    for amp, zone in _selected_zones(config, only):
        friendly = f"{amp['zones'][zone]} {suffix}"
        entity_config[f"media_player.{slugify(friendly)}"] = {
            "display_categories": "MUSIC_SYSTEM"
        }
    if not entity_config:
        raise ConfigError("No media-player zones were selected")
    return {"alexa": {"entity_config": entity_config}}


def render_package(config: dict[str, Any], only: list[str] | None = None) -> str:
    mode = config["home_assistant"]["entity_mode"]
    header = (
        f"# Generated by RTI AD-series MQTT Bridge v{VERSION}\n"
...
def render_alexa_cloud_snippet(
    config: dict[str, Any], only: list[str] | None = None
) -> str:
    header = (
        f"# Generated by RTI AD-series MQTT Bridge v{VERSION}\n"
        "# Merge this into a file included with: cloud: !include cloud.yaml\n"
        "# If configuring Cloud directly, nest this content beneath cloud:.\n"
        "# This sets Alexa device categories only; it does not filter or rename entities.\n"
    )
    return header + yaml.safe_dump(
        build_alexa_cloud_config(config, only), sort_keys=False, width=1000
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--alexa-output",
        help="Alexa Cloud snippet output (default: rti_ad8x_alexa_cloud.yaml beside --output)",
    )
    parser.add_argument(
    ...
    alexa_output = (
        Path(args.alexa_output).expanduser()
        if args.alexa_output
        else output.with_name("rti_ad8x_alexa_cloud.yaml")
    )
    existing = [path for path in (output, alexa_output) if path.exists()]
    ...
    try:
        ...
        alexa_rendered = render_alexa_cloud_snippet(config, args.zones)
    except ConfigError as exc:
        print(f"Package not written: {exc}", file=sys.stderr)
        return 2
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(rendered, encoding="utf-8")
    alexa_output.parent.mkdir(parents=True, exist_ok=True)
    alexa_output.write_text(alexa_rendered, encoding="utf-8")
    print(f"Home Assistant package written to {output}")
    print(f"Alexa Cloud snippet written to {alexa_output}")
    return 0
